...
def test(env, n_episodes, policy,device, render=False):
    ...
    for episode in range(n_episodes):
        ...
        total_reward = 0.0
        for t in count():
            action = policy(state.float().to(device),vec.float().to(device)).max(1)[1].view(1,1)
            # if action==0:
            #     action_=[-3.0,0]
            # elif action==1:
            #     action_=[-1.0,0]
...
if __name__=='__main__':
    ...
    params = {
      ...
      'discrete_acc': [-3.0, 1.0,2.0],  # discrete value of accelerations
      ...
      'out_lane_thres': 2.25,  # threshold for out of lane
      'desired_speed': 7,  # desired speed (m/s)
      'max_ego_spawn_times': 1000,  # maximum times to spawn ego vehicle
    ...
    env = make_env(env)
    agent = torch.load("saved_model_checkpoint")
